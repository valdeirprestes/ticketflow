...
export default function TicketsPage() {
  ...
  const buscarChamados = async (token: string, obj: Object) => {
    try {
      ...
      const response_abertos = await fetch(`${process.env.NEXT_PUBLIC_API}/chamado/solicitante?${query}`, {
                ...
                {popup.type === "comment" ? (
                  <div className="space-y-3 text-left">
                    ...
                    <div className="flex gap-2">
                      ...
                      className="w-1/2 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-xs py-2.5 rounded-xl transition">
                      Cancelar</button>
                      <button onClick={handleSubmitComment} 
                      disabled={!newComment.trim()} 
                      className="w-1/2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold text-xs py-2.5 rounded-xl transition shadow-md shadow-blue-100">
                        Enviar
                      </button>
                    </div>
                  </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              {tickets.map((ticket) => (
                ...
                <div
                  key={ticket.id}
                  onClick={() => router.push(`/tickets/${ticket.id}`)} // <-- AJUSTE AQUI: Redireciona para a tela de detalhes
